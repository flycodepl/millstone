...
function resolve(options, callback) {
    ...
    Step(function setup() {
        ...
        // For include in datasource
        resolved.Layer.forEach(function(i, index) {
          if (!i.Datasource || !i.Datasource.include) return
          for(var index in i.Datasource.include) {
              var name = i.Datasource.include[index];
              var filepath;
              if (name && name[0] === '/') {
                  filepath = name;
              } else {
                  filepath = path.resolve(path.join(base, name));
              }
              ...
              fs.readFile(filepath, function (err, data) {
                  ...
                  for(var keyname in jsondata) {
                      i.Datasource[keyname] = jsondata[keyname];
                  };
              });
          }
            delete i.Datasource.include;
          });

          //cheat - convert table_name to table
          resolved.Layer.forEach(function(i, index) {
              if (!i.Datasource || !i.Datasource.table_name || !i.Datasource.layer_type ) return
              var tn = i.Datasource.table_name;
              var type;
              switch (i.Datasource.layer_type.toLowerCase()) {
              case 'polygon':
              ...
              var tablename = "(SELECT * FROM \""+tn+"\" WHERE upper(geometrytype(\"geometry\")) IN ("+type+")) as \""+tn+"\"";

              i.Datasource.table = tablename;
              delete i.Datasource.table_name;
              delete i.Datasource.layer_type;
          });
